/*
 * udpsend - read line from stdin and send via UDP
 *
...
{
	printf("Usage: udpsend <ip> <port> [OPTIONS]\n");
	printf("\n");
	printf("  Send each line from stdin in a UDP packet\n");
	printf("\n");
	printf("Options:\n");
...
int main(int argc, char *argv[])
{
	...
	asn_loop_add_fd(0, input, NULL);
	udp_sender = asn_loop_udp_sender(bindif, ip, port);
